...
def train(rank, a, h):
    ...
    if h.num_gpus > 1:
        ...
        mrsd = DistributedDataParallel(mrsd, device_ids=[rank]).to(device)

    optim_g = torch.optim.AdamW(
        generator.parameters(), h.learning_rate, betas=[h.adam_b1, h.adam_b2]
    )
    optim_d = torch.optim.AdamW(
        itertools.chain(msd.parameters(), mpd.parameters(), mrsd.parameters()),
        h.learning_rate,
    ...
    for epoch in range(max(0, last_epoch), a.training_epochs):
        ...
        for i, batch in enumerate(train_loader):
            ...
            # MRSD
            y_dMRS_hat_r, y_dMRS_hat_g, _, _ = mrsd(y, y_g_hat.detach())
            loss_disc_MRS, losses_disc_s_r, losses_disc_s_g = discriminator_loss(
                y_dMRS_hat_r, y_dMRS_hat_g
            )

            loss_disc_all = loss_disc_s + loss_disc_f + loss_disc_MRS
            ...
            y_dMRS_hat_r, y_dmrs_hat_g, fmap_mrs_r, fmap_mrs_g = mrsd(y, y_g_hat)

            loss_fm_f = feature_loss(fmap_f_r, fmap_f_g)
            loss_fm_s = feature_loss(fmap_s_r, fmap_s_g)
            loss_fm_mrs = feature_loss(fmap_mrs_r, fmap_mrs_g)
            loss_gen_f, losses_gen_f = generator_loss(y_df_hat_g)
            loss_gen_s, losses_gen_s = generator_loss(y_ds_hat_g)
            loss_gen_mrs, losses_gen_mrs = generator_loss(y_dmrs_hat_g)
            loss_gen_all = (
                loss_gen_s
                + loss_gen_f
                + loss_gen_mrs
