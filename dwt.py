...
class DWT_1D(nn.Module):
    ...
    def initialization(self):
        self.filter_low = self.filt_low[None, None, :].repeat(
            (self.out_channels, self.in_channels // self.groups, 1)
        )
        self.filter_high = self.filt_high[None, None, :].repeat(
            (self.out_channels, self.in_channels // self.groups, 1)
        )
    ...
    def forward(self, input):
        ...
        return F.conv1d(
            input,
            self.filter_low.to(input.device),
            stride=self.stride,
            groups=self.groups,
        ), F.conv1d(
            input,
            self.filter_high.to(input.device),
            stride=self.stride,
            groups=self.groups,
        )
